...
if __name__ == "__main__":
    ...
    while True:
        ...
        if not output_directory_input:
            output_directory_base = script_dir
            print(f"Output directory set to default: {output_directory_base}")
            break
            ...
            try:
                ...
                output_directory_base = output_directory_input
                break
            except OSError as e:
    ...
    timestamp_str = current_datetime.strftime("%Y%m%d_%H%M%S") # Matches the GUI timestamp format

    output_folder_name = f"DirListHash_Out_{timestamp_str}"
    output_directory_final = os.path.join(output_directory_base, output_folder_name) 

    try:
        if not os.path.exists(output_directory_final):
            os.makedirs(output_directory_final)
        print(f"Reports will be saved to: {output_directory_final}")
        output_directory = output_directory_final 
    except Exception as e:
        print(f"Fatal Error: Could not create final output directory {output_directory_final}. Aborting. Error: {e}")
        sys.exit(1)
    ...
    base_filename_type = hash_choice if hash_choice != 'none' else 'listing'
    base_filename = f"directory_{base_filename_type}_{clean_path_for_filename}_{timestamp_str}"
